...
class TemplateGenerator {
  ...
  _create(options = {}) {
    ...
    if (options.isDir) {
      this._createDirectory(
        this._getDirPath(type),
        { name, actions, filesType },
        filesType
      );
    } else {
      const tpl = this._compileTpl(this._getSingleTpl(type), {
        name,
        actions,
        filesType,
      });
  ...
  _createFile(name, type, fileType, tpl) {
    fs.outputFile(this._createFilePath(name, type, fileType), tpl, function (
      err
    ) {
  ...
  _createFileName(tempFile, name, fileTypes) {
    ...
    if (newName.indexOf("tpl") > -1) {
      newName = newName
        .replace(/tpl/, "component")
        .replace(/extension/, fileTypes.html);
    }

    if (newName.indexOf("sty") > -1) {
      newName = newName
        .replace(/sty/, "component")
        .replace(/extension/, fileTypes.style);
    }

    if (newName.indexOf("script") > -1) {
      newName = newName
        .replace(/script/, "component")
        .replace(/extension/, fileTypes.script);
    }
